...
class Stat(object):
    ...
    def set_density_prior(self,Number_para = 70):
        '''
        ...
        self.density_prior =  tf.zeros(Number_para,self.tfdtype )
    ...
    def joint_log_post(self,mu,monitor=False):
        ...
        prior_log_prob = self.mvn_prior.log_prob(mu[:self.Number_para])

        joint_log = prior_log_prob_den+ prior_log_prob  + self.likelihood_log_prob
 
        if monitor == True:
            tf.print('prior:',prior_log_prob,'   likelihood:',self.likelihood_log_prob,'   joint posterior:',joint_log)
