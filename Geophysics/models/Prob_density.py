...
class Stat(object):
    ...
    def set_density_prior(self,mean,Number_para = 70,norm = None):
        '''
        ...
        self.density_prior = mean
        self.std_density = tf.ones(Number_para,self.tfdtype )
        self.norm = norm
    ...
    def set_lower_prior(self,mean,low_lower,high_lower,Number_para = 70,norm = None):
        '''
           prior distribution of the lower surface
        '''
        # define the model parameters
        self.Number_para = Number_para
    
        # define the prior distribution
        self.mu_prior_lower = mean
        self.std_lower = 1
        self.norm = norm
        self.low_lower = low_lower
    ...
    def joint_log_post(self,mu,monitor = False):
        ...
        # define density prior as a multivariate normal distribution      
        self.mvn_prior_density = tfd.MultivariateNormalDiag(
            loc=self.density_prior,
            scale_diag = self.std_density
            )
        ...
        self.mvn_prior_lower = tfd.TruncatedNormal(
            loc=self.mu_prior_lower,
            scale = self.std_lower,
            low = self.low_lower,
            high = self.high_lower,
            )
        ...
        prior_log_prob_den = self.mvn_prior_density.log_prob(mu[-3:])
        prior_log_prob_upper = self.mvn_prior_upper.log_prob(mu[self.Number_para:-3])
        prior_log_prob_lower = tf.reduce_sum(self.mvn_prior_lower.log_prob(mu[:self.Number_para]))

        joint_log = prior_log_prob_den + prior_log_prob_upper + prior_log_prob_lower + self.likelihood_log_prob
 
        if monitor == True:
            tf.print('prior_density:',prior_log_prob_den,'   prior_upper:',prior_log_prob_upper,'   prior_lower:',prior_log_prob_lower,'   likelihood:',self.likelihood_log_prob)
            # tf.print('prior_upper:',prior_log_prob_upper)
            # tf.print('prior_lower:',prior_log_prob_lower)
            # tf.print('likelihood:',self.likelihood_log_prob)
    ...
    def run_MCMC(self,method,
        ...
        if method == 'NUT':
            ...
            self.time_nut = end-start
            print('NUS time in seconds: %.3f' % (self.time_nut))
    ...
    def RMH(self,num_results,number_burnin,step_size,parallel_iterations = 1):
        ...
        states  = tfp.mcmc.sample_chain(
            ...
            # seed=42 # not supported in clustter version tfp
            )
    ...
    def NoUTurn(self,num_results,number_burnin,step_size):
        states  = tfp.mcmc.sample_chain(
            num_results=num_results,
            current_state=self.initial_chain_state,
            # trace_fn=None,
