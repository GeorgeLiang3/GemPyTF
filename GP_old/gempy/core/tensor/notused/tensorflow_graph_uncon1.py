...
class TFGraph(tf.Module):
    ...
    def __init__(self, input,
                ...
                Range, C_o, rescalefactor,rest_ref_indices,npf_indices_list,N_iteration,slope=50, output=None, **kwargs):
    ...
    def export_formation_block(self, Z_x, scalar_field_at_surface_points, values_properties,n_iter):
        ...
        #     tf.print('executing')
        #####
        ## TODO: run it first to define the Shape before the loop, could find a better way to do this
        ...
        # tf.print('formations_block:',self.formations_block)
        i0 = tf.constant(1)
        c = lambda i,formations_block: i < n_iter
        def body(i,formations_block):
            formations_block +=self.compare(scalar_field_iter[i], scalar_field_iter[i + 1],
                             2 * i, Z_x, slope, n_surface_op_float_sigmoid, drift)
            
            i_next = i+1
            return i_next,formations_block
        i0,self.formations_block = tf.while_loop(cond = c,
                      body=body,
                      loop_vars=[i0,self.formations_block],
                      shape_invariants = [i0.get_shape(),
                                          self.formations_block.get_shape()]
                      )
        # tf.print('i0:',i0)
        # tf.print('self.formations_block:',self.formations_block)
    ...
    def compute_series(self,surface_point_all,dips_position_all,dip_angles, azimuth, polarity,value_properties,):
        ...
        i = 0
        n_iter = tf.constant(self.N_iteration,tf.int32)[i]
        ...
        i0,block_matrix,property_matrix,mask_matrix = tf.while_loop(cond = c,
                      body=loop_compute_series,
                      loop_vars=[i0,block_matrix,property_matrix,mask_matrix],
                      shape_invariants = [i0.get_shape(),
