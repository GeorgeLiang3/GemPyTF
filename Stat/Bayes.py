...
class Stat_model(object):
    def __init__(self, Model,gravity_function,Number_para_total,tz,transformer = None) -> None:
        self.model = Model
        self.gravity_function = gravity_function
        self.monitor = False
        self.Number_para_total = Number_para_total
        self.static_xy = constant64(self.model.surface_points.df[['X','Y','Z']].to_numpy()[:,0:2])
        self.num_sfp_para=  self.model.surface_points.df[['X','Y','Z']].to_numpy().shape[0]
    ...
    def joint_log_post(self,mu,monitor=True):
        ...
        if monitor == True:
            tf.print('prior:',prior_log_prob,'   likelihood:',likelihood_log_prob,  '   posterior:',joint_log)
    ...
    def log_likelihood(self,mu):
        ...
        Gm_ = self.gravity_function(mu,self.model,self.tz,self.static_xy,self.sfp_shape,transformer = self.transformer)
    ...
    @tf.function
    def negative_log_posterior(self,mu,monitor=False):
        return tf.negative(self.joint_log_post(mu,monitor=monitor))

    @tf.function
    def negative_log_likelihood(self,mu):
        return tf.negative(self.log_likelihood(mu))
        
    @tf.function
    def likelihood(self,mu):
        return tf.math.exp(self.log_likelihood(mu))
    ...
    def Search_MAP(self,mu_init):
      @tf.function
      def loss_(mu):
          lost =  tf.negative(self.joint_log_post(mu,monitor = False))
          return lost

      @tf.function
      def loss_minimize():
          lost =  self.negative_log_posterior(mu_init)
          return lost
      ...
      def Adam(mu_init,learning_rate,iterations,mu_list,cost_A):
          ...
          for step in range(iterations):
              ...
              ## Directly apply the minize will take all the Variables into account, therefore we manually apply only the parameters we are interested at the moment
      ...
      learning_rate = Adamargs.learning_rate
      iterations = Adamargs.Adam_iterations
      ...
      # print('MAP:',MAP.numpy())
      return mu_list,cost_A,MAP.numpy()
